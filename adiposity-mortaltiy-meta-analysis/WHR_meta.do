///WHR subgroup data intergration///
///Categorical///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("categorical") firstrow clear
...
///Continous///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("continuous") firstrow clear
...
gen double dose01c = (dose - dose[1]) / 0.1

assert ref[1] == 1
assert abs(dose01c[1]) < 1e-10
assert abs(logrr[1]) < 1e-10
assert abs(selogrr[1]) < 1e-10

glst logrr dose01c, se(selogrr) cov(person_years case) ir


///linear///
/*HKSJ method*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("linear_main") firstrow clear
...
meta forestplot _id _plot _esci, leaveoneout random(reml) se(khartung) eform columnopts(_id, title("Study omitted")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(1 1.2 1.3) xscale(range(1 1.2 1.3)) nullrefline
...
/// Leave-one-out sensitivity analysis///
meta summarize, leaveoneout random(reml) se(khartung) eform
meta forestplot _id _plot _esci, leaveoneout random(reml) se(khartung) eform columnopts(_id, title("Study omitted")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(1 1.3 1.5) xscale(range(1 1.3 1.5)) nullrefline
...
///Sensitivity: restricted to continous and quantile based studies///
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("linear_main") firstrow clear
keep if bmi_unadjusted == 1
keep if minimum == 1
...
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("linear_open") firstrow clear
keep if bmi_unadjusted == 1
keep if minimum == 1
meta set coef se, studylabel(Studyname)
meta summarize, random(reml) se(khartung)
...
drop if inlist(id, 40, 41, 43, 44, 46)
duplicates report cohort sex
...
drop if inlist(id, 4, 17, 18, 43, 44, 46)
...
destring(older_age), replace
drop if older_age == .
drop if inlist(id, 14, 15, 40, 41, 47, 60)
...
keep if healthy == 1
drop if inlist(id, 5, 6, 10, 21, 22, 46, 60, 65, 66)
...
gen double rug_all_y = 1 if inrange(dose, 0.7, 1.1)
gen double rug_ref_y = 1 if refcat & inrange(dose, 0.7, 1.1)

	
drmeta_graph, matk(knots) dose(0.7(0.001)1.1) list ref(`p25') eform format(%9.6f) ///
    xtitle("WHR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
    xlabel(0.7(0.1)1.1, labsize(large)) ///
...
drmeta_graph, matk(knots) dose(0.7(0.001)1.1) list ref(`p25') eform ///
    xtitle("WHR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
/*Open ended*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHR\WHR_meta.xlsx", sheet("categorical_open") firstrow clear
...
gen double fit_lo = ///
    exp(fit_logrr - invnormal(.975) * sqrt(fit_var))
...
capture drop refcat rug_all_y rug_ref_y
...
twoway ///
    ...
    (line fit_rr_plot dose_curve if curveobs == 1, ///
        ...
        lpattern(solid) ///
        lwidth(medium)) ///
    (scatter rug_all_y dose ///
        if curveobs == 0 & !missing(rug_all_y), ///
        msymbol(|) ///
        mcolor(gs10) ///
        msize(medium)) ///
    (scatter rug_ref_y dose ///
        if curveobs == 0 & !missing(rug_ref_y), ///
        msymbol(|) ///
        mcolor(black) ///
        msize(large)), ///
    xtitle("WHR", size(large)) ///
    ...
    ylabel(0.6 0.8 1.0 1.5 2.0 2.5 3.0 3.5 4.0, ///
        ...
        angle(horizontal) ///
        norescale) ///
    xscale(range(0.70 1.1)) ///
    yscale(log range(0.60 4.00)) ///
    yline(1, ///
        lcolor(gs10) ///
        lpattern(solid)) ///
    legend(off) ///
...
local search_min 0.7
local search_max 1.1
...
drmeta_graph, matk(knots) dose(0.7(0.001)1.1) list ref(`nadir') eform ///
    xtitle("WHR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
local search_min 0.70
local search_max 1.10
local gridstep   0.0001
...
local ngrid = ///
    round((`search_max' - `search_min') / `gridstep') + 1
...
gen double dose_grid = ///
    `search_min' + `gridstep' * (_n - 1)

mkspline nad = dose_grid, ///
    knots(`knot1' `knot2' `knot3') ///
    cubic
...
twoway ///
    (line fit_hi_plot dose_curve ///
        if curveobs == 1, ///
        sort ///
        lcolor(black) ///
        lpattern("-#") ///
        lwidth(medium)) ///
    (line fit_lo_plot dose_curve ///
        if curveobs == 1, ///
        sort ///
        lcolor(black) ///
        lpattern("-#") ///
        lwidth(medium)) ///
    (line fit_rr_plot dose_curve ///
        if curveobs == 1, ///
        sort ///
        lcolor(black) ///
        lpattern(solid) ///
...
drop if missing(id, dose, RR, LCI, UCI, logrr, selogrr)
keep if ad_cov_cat == 2 /*2-3 (=1) and above 4 (=2)*/
...
gen double rug_all_y = 1 if inrange(dose, 0.7, 1)
gen double rug_ref_y = 1 if refcat & inrange(dose, 0.7, 1)
