///WHtR subgroup data intergration///
///Categorical///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("categorical") firstrow clear
...
///continuous///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("continous") firstrow clear
...
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("continuous") firstrow clear
...
/*HKSJ method*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("linear_main") firstrow clear
...
/// Leave-one-out sensitivity analysis///
meta summarize, leaveoneout random(reml) se(khartung) eform
meta forestplot _id _plot _esci, leaveoneout random(reml) se(khartung) eform columnopts(_id, title("Study omitted")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(1 1.5 2.0) xscale(range(1 1.5 2.0)) nullrefline
...
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("linear_open") firstrow clear
keep if bmi_unadjusted == 1
keep if minimum == 1
meta set coef se, studylabel(Studyname)
meta summarize, random(reml) se(khartung)
...
drop if inlist(id, 11, 12, 19, 20, 21, 22, 23, 46, 47)
duplicates report cohort sex
...
meta set coef se, studylabel(Studyname)
meta summarize, subgroup(ad_cov_cat) random(reml) se(khartung) eform
meta forestplot _id _plot _esci _weight, subgroup(ad_cov_cat) random(reml) se(khartung) eform columnopts(_id, title("Author, year")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(0.5 1 1.5 2) xscale(range(0.5 2))nullrefline
...
meta set coef se, studylabel(Studyname)
meta summarize, subgroup(age_group) random(reml) se(khartung) eform
meta forestplot _id _plot _esci _weight, subgroup(age_group) random(reml) se(khartung) eform columnopts(_id, title("Author, year")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(0.5 1 1.5 2) xscale(range(0.5 2))nullrefline
...
keep if healthy == 1
drop if inlist(id, 2, 3, 8, 9, 10, 11, 12, 26, 27, 40, 41, 45, 46, 47, 50, 52, 53, 54, 55)
...
gen double rug_all_y = 1 if inrange(dose, 0.35, 0.65)
gen double rug_ref_y = 1 if refcat & inrange(dose, 0.35, 0.65)

	
drmeta_graph, matk(knots) dose(0.35(0.001)0.65) list ref(`nadir') eform format(%9.6f) ///
    xtitle("WHtR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
    xlabel(0.35(0.05)0.65, labsize(large)) ///
...
drmeta_graph, matk(knots) dose(0.35(0.001)0.65) list ref(`p25') eform format(%9.6f) ///
    xtitle("WHtR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
drmeta_graph, matk(knots) dose(0.35(0.001)0.65) list ref(`nadir') eform ///
    xtitle("WHtR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
/*Open ended*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WHtR\WHtR_meta_final.xlsx", sheet("categorical_open") firstrow clear
...
drmeta_graph, matk(knots) dose(0.35(0.001)0.65) list ref(`p25') eform ///
    xtitle("WHtR", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
local search_min  0.35
local search_max 0.7
local gridstep     0.001
...
foreach y in fit_rr fit_lo fit_hi {
    ...
    replace `y'_plot = . ///
        if (`y' > `ymax' & !boundary_hi_`y') | ///
           (`y' < `ymin' & !boundary_lo_`y')
}
...
gen double rug_all_y = 1 if inrange(dose, 0.35, 0.7)
gen double rug_ref_y = 1 if refcat & inrange(dose, 0.35, 0.65)
...
keep if full_model == 1
drop if mediator == 1
drop if inlist(id, 2, 3, 17, 18, 22, 23, 24, 27)
drop if missing(id, dose, RR, LCI, UCI, logrr, selogrr)
