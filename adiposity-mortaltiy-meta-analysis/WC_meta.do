///WC subgroup data intergration///
///Categorical///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("categorical_raw") firstrow clear
...
///Continous///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("continous_only_10cm") firstrow clear

keep if inlist(id, 37, 38)
...
///GLST///
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("categorical") firstrow clear
...
/*HKSJ method*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("linear_main") firstrow clear
...
meta forestplot _id _plot _esci _weight, random(reml) se(khartung) eform ///
...
///Leave-one-out sensitivity analysis: REML + HKSJ///
meta summarize, leaveoneout random(reml) se(khartung) eform
meta forestplot _id _plot _esci, leaveoneout random(reml) se(khartung) eform columnopts(_id, title("Study omitted")) columnopts(_esci, supertitle("RR") title("(95% CI)")) xlabel(1 1.05 1.10) xscale(range(1 1.06 1.10)) nullrefline
...
///Sensitivity: excluding non-quantile based / qunatile based studies///
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("linear_main") firstrow clear
keep if bmi_unadjusted == 1
keep if minimum == 1
keep if quantile_based == 1 | continous == 1
keep if full_model == 1
meta set coef se, studylabel(Studyname)
meta summarize, random(reml) se(khartung)
...
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("linear_open") firstrow clear
keep if bmi_unadjusted == 1
keep if minimum == 1
meta set coef se, studylabel(Studyname)
meta summarize, random(reml) se(khartung)
...
drop if inlist(id, 21, 22, 26, 62, 63, 65, 66, 113, 114)
duplicates report cohort sex
...
drop if inlist(id, 29, 37, 43, 44, 51, 58, 65, 66, 78, 109)
...
drop if older_age < 50
drop if inlist(id, 15, 16, 35, 36, 43, 44, 56, 57, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 125, 126, 127, 128)
...
drop if inlist(id, 35, 36, 44, 56, 57, 62, 63, 64, 65, 66, 125, 126, 127, 128)
...
///Including studies without core confounders///
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("linear_main") firstrow clear
keep if bmi_unadjusted == 1
keep if full_model == 1
...
gen double rug_all_y = 1 if inrange(dose, 60, 120)
gen double rug_ref_y = 1 if refcat & inrange(dose, 60, 120)

	
drmeta_graph, matk(knots) dose(60(0.1)120) list ref(`nadir') eform format(%9.6f) ///
    xtitle("WC (cm)", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
    xlabel(60(10)120, labsize(large)) ///
    yscale(range(0.6 0.8 3.0))
...
drmeta_graph, matk(knots) dose(60(0.1)120) list ref(`p25') eform format(%9.6f) ///
    xtitle("WC (cm)", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
drmeta_graph, matk(knots) dose(60(0.1)120) list ref(`nadir') eform ///
    xtitle("WC (cm)", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
twoway (line fit_hi_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern("-#") lwidth(medthick))(line fit_lo_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern("-#") lwidth(medthick))(line fit_rr_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern(solid) lwidth(medium))(scatter rug_all_y dose if curveobs == 0 & !missing(rug_all_y),  msymbol(|) mcolor(gs10) msize(medium)) (scatter rug_ref_y dose if curveobs == 0 & !missing(rug_ref_y), msymbol(|) mcolor(black) msize(large)), xtitle("WC (cm)", size(large)) ytitle("RR", size(large))xlabel(60(10)120, labsize(large) norescale)ylabel(0.6 0.8 1.0 1.5 2.0 2.5 3.0 3.5 4.0, format(%3.1f) labsize(large) angle(horizontal) norescale) xscale(range(60 120)) yscale(log range(0.6 4.0)) yline(1, lcolor(gs10) lpattern(solid)) legend(off) graphregion(color(white)) plotregion(color(white))

restore


/*=====================Sesntivity for open ended categry========================*/
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("categorical_open") firstrow clear
...
drmeta_graph, matk(knots) dose(60(0.1)120) list ref(`p25') eform ///
    xtitle("WC (cm)", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
...
display as text "Estimated nadir = " as result %6.2f `nadir' as text " cm"
...
gen double dose_curve = `xmin' + `step' * (_n - 1) in 1/`ngrid'
...
mkspline sp = dose_curve, knots(`k1' `k2' `k3') cubic
...
gen double fit_logrr = scalar(`b1') * d1 + scalar(`b2') * d2

gen double fit_var = d1^2 * scalar(`v11') + d2^2 * scalar(`v22')  + 2*d1*d2 * scalar(`v12')

replace fit_var = 0 if fit_var < 0 & fit_var > -1e-12
...
foreach y in fit_rr fit_lo fit_hi {
    ...
    gen byte boundary_`y' = `y' > `ymax' & ((_n > 1  & `y'[_n-1] <= `ymax') | (_n < _N & `y'[_n+1] <= `ymax'))

    gen double `y'_plot = `y'

    replace `y'_plot = `ymax' if boundary_`y'

    replace `y'_plot = .  if `y' > `ymax' & !boundary_`y'
}

gen byte curveobs = 1

keep dose_curve fit_rr_plot fit_lo_plot fit_hi_plot curveobs
...
capture confirm variable rug_all_y
if _rc {
    gen double rug_all_y = 1 if inrange(dose, `xmin', `xmax')
}

capture confirm variable rug_ref_y
if _rc {
    gen double rug_ref_y = 1 if refcat & inrange(dose, `xmin', `xmax')
}
...
twoway (line fit_hi_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern("-#") lwidth(medium))(line fit_lo_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern("-#") lwidth(medium))(line fit_rr_plot dose_curve if curveobs == 1, sort lcolor(black) lpattern(solid) lwidth(medium))(scatter rug_all_y dose if curveobs == 0 & !missing(rug_all_y),  msymbol(|) mcolor(gs10) msize(medium)) (scatter rug_ref_y dose if curveobs == 0 & !missing(rug_ref_y), msymbol(|) mcolor(black) msize(large)), xtitle("WC (cm)", size(large)) ytitle("RR", size(large))xlabel(60(10)120, labsize(large) norescale)ylabel(0.6 0.8 1.0 1.5 2.0 2.5 3.0 3.5 4.0, format(%3.1f) labsize(large) angle(horizontal) norescale) xscale(range(60 120)) yscale(log range(0.6 4.0)) yline(1, lcolor(gs10) lpattern(solid)) legend(off) graphregion(color(white)) plotregion(color(white))
...
gen double rug_all_y = 1 if inrange(dose, 60, 120)
gen double rug_ref_y = 1 if refcat & inrange(dose, 65, 120)
...
keep if sex == 2
keep if minimum == 1
drop if inlist(id, 16, 22, 23, 24, 38, 48, 81)
drop if missing(id, dose, RR, LCI, UCI, logrr, selogrr)
...
drmeta logrr doses*, se(selogrr) data(person_years case) id(id) type(drtype) reml eform
...
replace early_death_grp = 0 if early_death < 1
replace early_death_grp = 1 if early_death >= 1
keep if early_death_grp == 1
drop if inlist(id, 76, 79, 80, 81)
drop if missing(id, dose, RR, LCI, UCI, logrr, selogrr)
...
///////================Restriction to older population=========================///////
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("categorical") firstrow clear
...
keep if minimum == 1
destring(older_age), replace
keep if older_age >= 60
drop if missing(older_age)
drop if missing(id, dose, RR, LCI, UCI, logrr, selogrr)
...
gen double rug_all_y = 1 if inrange(dose, 65, 120)
gen double rug_ref_y = 1 if refcat & inrange(dose, 65, 120)


drmeta_graph, matk(knots) dose(65(0.1)120) list ref(`p25') eform ///
    xtitle("WC (cm)", size(large)) ///
    ytitle("RR", size(large)) ///
    yline(1, lcolor(gs10) lpattern(line)) ///
    ylabel(0.6 0.8 1 (0.5) 4.0, labsize(large)) ///
    xlabel(65(5)120, labsize(large)) ///
    yscale(range(0.6 0.8 4.0))
...
//////=============Excluding studies adjusted for intermediate=================///////
/*BMI unadjusted*/
import excel "C:\Users\Jin Eui Kim\OneDrive\obesity_meta_data\WC\WC_meta_final.xlsx", sheet("categorical") firstrow clear
